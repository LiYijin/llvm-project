...
Type adaptIntegralTypeSignedness(Type ty, bool needsUnsigned) {
  if (isa<IntegerType>(ty)) {
    ...
    if (isa<emitc::SizeTType>(ty) != needsUnsigned) {
      ...
      // 不知道为啥要引入这个PtrDiffTType, 还是先暂时弄成int32_t吧；
      // return emitc::PtrDiffTType::get(ty.getContext());
      // 对于size_t到signed int的变换，变成了使用int32_t
      return IntegerType::get(ty.getContext(), 32, IntegerType::SignednessSemantics::Signed);
    }
  }
  return ty;
}
...
public:
  ...
  // copied from newer llvm version. Because the matchAndRewrite() depend on it.
  mlir::TypedAttr getOneAttr(ConversionPatternRewriter &rewriter, Type type) const {
  ...
  matchAndRewrite(ArithOp op, typename ArithOp::Adaptor adaptor,
    ...
    if (opReturnType.isInteger(1)) {
      Type attrType = (emitc::isPointerWideType(operandType))
                          ...
                          : operandType;
      auto oneAttr = getOneAttr(rewriter, attrType);
      auto constOne = rewriter.create<emitc::ConstantOp>(
          op.getLoc(), operandType, oneAttr);
...
public:
  ...
  matchAndRewrite(ArithOp op, typename ArithOp::Adaptor adaptor,
    ...
    // TODO: NOTE: I comment out here, because our arith has no `arith::IntegerOverflowFlags::nsw`.
